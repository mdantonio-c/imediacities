...
export class AppAnnotationsService {
  ...
  private confirmationPopover_config = {
    popoverTitle: "Delete",
    popoverMessage:
      "This operation cannot be undone<br>Do you wish to continue?",
    appendToBody: true,
  };
  ...
  delete_request(annotation, media_type) {
    let confirm = window.confirm(
      'Delete the annotation "' + annotation.name + '"?'
    );
    if (confirm) {
  ...
  delete_tag(annotation, media_type) {
    const bodyRef = annotation.iri
      ? `resource:${annotation.iri}`
  ...
  merge(shots, gruppo) {
    ...
    shots.forEach((s) => {
      s.annotations[gruppo].forEach((t) =>
        mappa.set(t.id + "_" + t.body_id, t)
      );
    });

    return Array.from(mappa)
      .map((t) => t[1])
      .sort((a, b) => {

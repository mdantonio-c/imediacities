...
export class AppAddToListComponent {
  ...
  private loadMyLists() {
    this.listsService.getLists(this.item_id).subscribe(
      (resp) => {
  ...
  private removeFromList(listId) {
    console.log(
      `remove <${this.item_type}:${this.item_id}> to the list <${listId}>`
    );
    this.listsService.removeItemfromList(this.item_id, listId).subscribe(
      (resp) => {
      ...
      (error) => {
        console.error(
          "There was an error removing item from the list",
          error.errors
        );

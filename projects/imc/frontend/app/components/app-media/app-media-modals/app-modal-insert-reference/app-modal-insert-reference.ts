...
export class AppModalInsertReferenceComponent {
  ...
  save() {
    ...
    if (form.book_title && form.book_title.trim() !== "") {
      reference.book_title = form.book_title.trim();
    }
    if (form.journal && form.journal.trim() !== "") {
      reference.journal = form.journal.trim();
    }
    if (form.volume && form.volume.trim() !== "") {
      reference.volume = form.volume.trim();
    }
    if (form.number && form.number.trim() !== "") {
      reference.number = form.number.trim();
    }
    if (form.year) {
      reference.year = form.year;
    }
    if (form.month) {
      reference.month = form.month;
    }
    if (form.editor && form.editor.trim() !== "") {
      reference.editor = form.editor.trim();
    }
    if (form.publisher && form.publisher.trim() !== "") {
      reference.publisher = form.publisher.trim();
    }
    if (form.address && form.address.trim() !== "") {
      reference.address = form.address.trim();
    }
    if (form.url && form.url.trim() !== "") {
      reference.url = form.url.trim();
    }
    if (form.isbn && form.isbn.trim() !== "") {
      reference.isbn = form.isbn.trim();
    }
    if (form.doi && form.doi.trim() !== "") {
      reference.doi = form.doi.trim();
    }

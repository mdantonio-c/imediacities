...
export class AppModalMoveCutComponent implements AfterViewInit, OnChanges, OnDestroy {
	...
	private change_cut(newFrameIdx: number): void {
		...
		if (this.shots[1].start_frame_idx !== this.current_cut) {
			...
			this.shots[1].timestamp = this.shotRevisionService.shot_timestamp(this.current_cut, this.fps);
			// should update duration
			this.shots[0].duration = this.shotRevisionService.shot_duration(this.shots[0], this.fps);
			this.shots[1].duration = this.shotRevisionService.shot_duration(this.shots[1], this.fps);
		}
	...
	ngOnChanges() {
		...
		if (this.data.shots.length === 1) {
			...
			this.current_cut = 
				shot.start_frame_idx + Math.floor((shot.end_frame_idx - shot.start_frame_idx) / 2) + 1;
			// split the shot

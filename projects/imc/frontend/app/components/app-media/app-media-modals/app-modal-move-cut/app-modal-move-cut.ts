...
export class AppModalMoveCutComponent
  ...
  constructor(
    ...
    this.subscription = shotRevisionService.cutChanged$.subscribe((cut) => {
      this.change_cut(cut);
    });
  }
  ...
  private change_cut(newFrameIdx: number): void {
    if (this.current_cut === newFrameIdx) {
      return;
    }
    this.current_cut = newFrameIdx;
    if (this.shots[1].start_frame_idx !== this.current_cut) {
      this.changed = true;
      this.shots[0].end_frame_idx = this.current_cut - 1;
      this.shots[1].start_frame_idx = this.current_cut;
      // should update timestamp
      this.shots[1].timestamp = this.shotRevisionService.shot_timestamp(
        this.current_cut,
        this.fps
      );
      // should update duration
      this.shots[0].duration = this.shotRevisionService.shot_duration(
        this.shots[0],
        this.fps
      );
      this.shots[1].duration = this.shotRevisionService.shot_duration(
        this.shots[1],
        this.fps
      );
    }
  ...
  private update_shots(): void {
    this.data.shots[0].end_frame_idx = this.shots[0].end_frame_idx;
    this.data.shots[0].duration = this.shots[0].duration;
    if (this.split) {
      this.shotRevisionService.addCut(this.shots);
      return;
    }

    this.data.shots[1].start_frame_idx = this.shots[1].start_frame_idx;
    this.data.shots[1].timestamp = this.shots[1].timestamp;
    this.data.shots[1].duration = this.shots[1].duration;
    this.data.shots[1].revision_confirmed = true;
  }
  ...
  ngOnChanges() {
    ...
    if (this.data.shots.length === 1) {
      ...
      this.current_cut =
        shot.start_frame_idx +
        Math.floor((shot.end_frame_idx - shot.start_frame_idx) / 2) +
        1;
      // split the shot
      ...
      this.current_cut = this.shots[1].start_frame_idx;
      this.changed = false;
      this.split = false;
    }
    if (this.modal.videoPlayer) {
      this.init_player();
    }
  }
  ...
  private split_shot(shot: any, cut: number): any[] {
    ...
    // update shot 1
    shot_1.end_frame_idx = cut - 1;
    shot_1.duration = this.shotRevisionService.shot_duration(shot_1, this.fps);
    // create a new shot 2
    let shot_2 = {
      id: null,
      links: null,
      attivo: false,
      shot_num: shot_1.shot_num + 1,
      start_frame_idx: cut,
    ...
    shot_2.duration = this.shotRevisionService.shot_duration(shot_2, this.fps);
    return [shot_1, shot_2];
  }
  ...
  private init_player() {
    this.modal.videoPlayer.range.set({
      index: this.shots[0].shot_num,
      start: this.shots[0].start_frame_idx,
      end: this.shots[1].end_frame_idx,
      loop: true,

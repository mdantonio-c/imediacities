...
export class AppModalTagCloudComponent implements OnInit, OnChanges, OnDestroy {
  ...
  shots_show(event, annotation) {
    let shots = this.data.shots.filter((s) =>
      this.ShotsService.shot_has_tag(s, annotation)
    );
  ...
  annotations_filter(event) {
    this.annotations_filtered.alpha = this.ShotsService.annotations_filter(
      this.annotations_sorted.alpha,
      event.target.value
    );
    this.annotations_filtered.count = this.ShotsService.annotations_filter(
      this.annotations_sorted.count,
      event.target.value
    );
  }

  ngOnInit() {
    this._subscription = this.ShotsService.update.subscribe(

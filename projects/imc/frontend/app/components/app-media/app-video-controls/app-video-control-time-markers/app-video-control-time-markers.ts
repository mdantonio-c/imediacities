...
export class AppVideoControlTimeMarkersComponent
  ...
  private static _floor(Number, DecimalPlaces) {
    return (
      Math.floor(parseFloat(Number) * Math.pow(10, DecimalPlaces)) /
  ...
  private _format(time, fps, smtpe = true) {
    ...
    this.hours = this._pad(
      AppVideoControlTimeMarkersComponent._floor(time / 3600, 0) % 24,
      2
    );
    this.minutes = this._pad(
      AppVideoControlTimeMarkersComponent._floor(time / 60, 0) % 60,
      2
    );
    this.seconds = this._pad(
      AppVideoControlTimeMarkersComponent._floor(time % 60, 0),
      2
    );
    this.smtpe = this._pad(
      AppVideoControlTimeMarkersComponent._floor(
        ((time % 1) * fps).toFixed(3),
        0
      ),
      2
    );
  ...
  private static _float_time(time) {
    return (
      Math.round(parseFloat(time) * Math.pow(10, 10)) / Math.pow(10, 10)
    ).toFixed(4);
  }

  public markers_update() {
    this._format(this.video.currentTime, this.parent.fps);
    this.float_mark = this._pad(
      AppVideoControlTimeMarkersComponent._float_time(this.video.currentTime),
      9
    );

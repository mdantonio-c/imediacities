...
if __name__ == "__main__":
    ...
    video_testfile_path = os.path.join(os.path.abspath(os.path.curdir), 'SampleVideo_640x360_1mb.mp4')
    ...
    if not os.path.exists(video_testfile_path):
        ...
        response = urllib.request.urlopen("https://seafile.idmt.fraunhofer.de/f/4d62eed6de/?raw=1")
    ...
    if not cap.isOpened():
        raise RuntimeError('Could open capture device - OpenCV might not have been compiled with FFMPEG support!')
    frame_count = 0
    while cap.isOpened():
        ret, frame = cap.read()

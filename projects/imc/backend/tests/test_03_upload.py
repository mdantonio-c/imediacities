...
class TestApp(BaseTests):
    ...
    def test_upload(self, client): #client e' una fixture di pytest-flask
        ...
        roles =[
        {'description': 'Role allowed to upload contents and metadata' , 'name': 'Archive'},
        ...
        post_md_data = {'file': ('tests/custom/data/test_md_1234.xml', 'test_md_1234.xml'),
            ...
            'flowTotalChunks':1,
            'flowChunkSize':1}
        res = client.post('/api/upload', headers=headers, content_type='multipart/form-data', data=post_md_data)
        ...
        post_video_data = {'file': ('tests/custom/data/test_video_1234.mp4', 'test_video_1234.mp4'),
            ...
            'flowTotalChunks':1,
            'flowChunkSize':1}
        res2 = client.post('/api/upload', headers=headers, content_type='multipart/form-data', data=post_video_data)
        ...
        post_filetodel_data = {'file': ('tests/custom/data/to_del.txt', 'to_del.txt'),
            ...
            'flowTotalChunks':1,
            'flowChunkSize':1}
        res3 = client.post('/api/upload', headers=headers, content_type='multipart/form-data', data=post_filetodel_data)

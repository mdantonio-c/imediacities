...
def _has_rights(user, entity_id):
    """
...
def _has_public_access(user, entity_id):
    """
...
def check_permissions(
    ...
    """Verify if user has permission to access the specified content id"""
    entity_id = video_id or image_id

    if entity_id is None:
        # Do not raise any exception but ignore it
        return {}

    if content_type is None or (content_type != "image" and content_type != "video"):
        # Do not raise any exception but ignore it
        return {}

    if not _has_public_access(user, entity_id):
        raise Forbidden("User is not authorized to access content")

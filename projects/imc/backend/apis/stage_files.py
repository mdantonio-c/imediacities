    ...
    def post(self):
        ...
        try:
            ...
            if len(c) > 1:
                ...
                log.errors("Database incoherence: there are more than one MetaStage related to the same source id %s" % source_id)
                raise RestApiException(
            ...
            if len(c) == 1:
                ...
                if meta_stage is not None:
                    ...
                    if filename != dbFilename:
                        raise RestApiException(
                            "Source id %s already esists in database but with different filename %s: cannot proceed with import!" % (source_id,dbFilename),
                            status_code=hcodes.HTTP_BAD_CONFLICT)
            ...
            if len(c) == 0:
                ...
                log.debug("Source id %s does not exist in database: creating new element" % source_id)
                ...
                if filename != standard_filename:
                    log.debug("File %s has not standard file name: renaming it to standard %s" % (filename,standard_filename))
                ...
                try:
                    ...
                    log.debug("MetaStage already exists for file %s, meta_stage.uuid=%s" % (path,meta_stage.uuid))
                    ...
                    log.debug("MetaStage created for %s, meta_stage.uuid=%s" % (path,meta_stage.uuid))

	  ...
	  $scope.open = function(size, template) {
	      modalInstance = $uibModal.open({
			  ...
			  resolve: {
                   params: function () {
                   	  var params = {};
        			  angular.extend(params, {'videoid' : self.selectedVideoId});
                      return params;
                   }
               }
	        });
  ...
  /*var address = 'Bologna, It';
	...
	var geocoder = new google.maps.Geocoder();
		...
		//self.video.location[0].lat = Lat;
		//self.video.location[0].lng = Lng;
		...
		/*filling the storyboard with the image shots*/
			...
			function(out_data) {
				...
				for (var i=0; i<self.shots.length; i++) { 
				    ...
				    var start_frame = self.shots[i].attributes.start_frame_idx;
				    var frameshot = [2];
					$scope.companies.push({ 'thumb': thumblink, 'number': i, 'duration': start_frame, 'camera': 'zoom out' }); 
				}
			});
